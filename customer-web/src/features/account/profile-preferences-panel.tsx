  ...
  return (
    <AppCard className="p-4">
      <div className="space-y-4">
        <SectionHeader
          eyebrow="Hồ sơ"
          title="Hồ sơ và sở thích khách hàng"
          description="Các trường tài khoản hiện lấy từ phiên khách hàng. Chỉnh sửa sở thích cần API hồ sơ khách hàng trước khi lưu."
          action={<StatusPill label="Cần API hồ sơ" tone="warning" />}
        />
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <ReadonlyPreference icon={MapPin} label="Chi nhánh yêu thích" value="Chưa kết nối" />
          <ReadonlyPreference icon={UsersRound} label="Số khách thường đi" value="Chưa kết nối" />
          <ReadonlyPreference icon={Clock3} label="Giờ ăn thường chọn" value="Chưa kết nối" />
        </div>
          ...
          {preferenceItems.map((item) => {
            ...
            return (
              <div key={item.label} className="rounded-lg border border-dashed bg-secondary/25 p-3">
                ...
                <p className="mt-2 text-xs text-muted-foreground">Đang chờ API sở thích</p>
              </div>
            );
          })}
        </div>
        <AppTextarea
          ...
          value=""
          disabled
          placeholder="Chỉ thu thập ghi chú khi API lưu hồ sơ đã sẵn sàng."
          helperText="Nền tảng này không lưu dữ liệu sở thích nhạy cảm trên trình duyệt."
        />
      </div>
    </AppCard>
  );
