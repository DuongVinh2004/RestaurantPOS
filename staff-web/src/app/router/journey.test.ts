...
describe('journey helpers', () => {
  ...
  it('builds a resume target that preserves the current operational context', () => {
    expect(buildJourneyResumeTarget({
      ...
      path: `${staffRoutePaths.ops.checkout}?source=checkout&table_id=18&table_ids=18%2C21&reservation_id=24&reservation_row_version=7&order_id=33&order_row_version=5`,
  ...
  it('builds a refund resume target when the route is already focused on refund work', () => {
    expect(buildJourneyResumeTarget({
      ...
      path: `${staffRoutePaths.ops.refunds}?source=refund&reservation_id=24&reservation_row_version=7&order_id=33&order_row_version=5`,
  ...
  it('falls back to a safe route when the source is absent but context remains', () => {
    expect(buildJourneyResumeTarget({
      ...
      path: `${staffRoutePaths.ops.reservations}?reservation_id=24&reservation_row_version=7`,
  ...
  it('keeps audit as a trace source while resuming the concrete operational route', () => {
    expect(buildJourneyResumeTarget({
      ...
      path: `${staffRoutePaths.ops.reservations}?source=audit&reservation_id=24`,
      label: 'Tiếp tục đặt bàn',
    });
  });
