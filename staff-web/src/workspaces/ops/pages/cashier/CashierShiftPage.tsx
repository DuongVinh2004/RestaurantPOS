...
export function CashierShiftPage() {
    ...
    <div className="staff-workspace-fluid staff-workspace-flex-column" style={{ display: 'flex', flexDirection: 'column', gap: '16px', width: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', background: '#fff', padding: '12px 16px', borderRadius: '8px', border: '1px solid #f0f0f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <Typography.Title level={4} style={{ margin: 0 }}>Trung tâm ca thu ngân</Typography.Title>
          <StatusChip
      ...
      {requiresCashierShift && !currentShift ? (
        <Alert
          type="warning"
          showIcon
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={14}>
          <Space orientation="vertical" size={16} style={{ width: '100%' }}>
            <Card title="Mở ca thu ngân" className="staff-workspace-form-card">
        ...
        <Col xs={24} lg={10}>
          <Card title="Chi tiết & Đóng ca" className="staff-workspace-detail-card" style={{ height: '100%' }}>
            {!selectedShift ? (
              <EmptyBlock
                title="Chưa chọn ca"
  ...
  return (
    <div data-testid="cashier-shift-page" style={{ padding: '16px', background: '#f5f7fa', minHeight: '100%', width: '100%' }}>
      {main}
    </div>
  );
