...
import { resolveWorkspaceNavigationSections } from '../../../../app/router/navigation';
...
import { buildJourneyResumeTarget } from '../../../../app/router/journey';
import { paymentTone } from '../../../../shared/status/status';
...
export function AccessGatePage() {
  ...
  const navigationSections = resolveWorkspaceNavigationSections(session);
  const navigationItems = navigationSections.flatMap((section) => section.items);
  const cashierShiftAvailable = navigationItems.some((item) => item.key === 'cashier-shift');
  ...
  return (
    <Space orientation="vertical" size={20} style={{ width: '100%' }}>
      ...
      <div className="staff-task-grid">
        <Card className="staff-task-card staff-task-card-primary" title="Bước nên làm ngay">
          <Space orientation="vertical" size={12} style={{ width: '100%' }}>
            ...
            <div className="staff-action-row">
              ...
              {financeBlocked && cashierShiftAvailable && recommendedItem?.key !== 'cashier-shift' ? (
                <Button onClick={() => navigate(staffRoutePaths.ops.cashierShift)}>
                  Mở ca thu ngân
                </Button>
              ) : null}
            </div>
        ...
        <Card className="staff-task-card" title="Workspace access">
          <Space orientation="vertical" size={16} style={{ width: '100%' }}>
            {navigationSections.map((section) => (
              <div key={section.workspace}>
                ...
                <div className="staff-task-list">
                  {section.items.map((item) => {
                    const blockedByShift = financeBlocked && (item.key === 'checkout' || item.key === 'finance-review');
                    ...
                    return (
                      <div key={item.path} className="staff-task-list-item">
                        <div className="staff-task-list-copy">
                          ...
                          <Typography.Text type="secondary">
                            {blockedByShift ? `${item.description} Requires an open cashier shift first.` : item.description}
                          </Typography.Text>
                        </div>
                        <Button
                          key={item.path}
                        ...
                        >
                          {blockedByShift ? 'Open shift first' : 'Open'}
                        </Button>
                      </div>
                    );
                  })}
