...
export function AdminSettingsPage() {
  ...
  return (
    <div className="staff-workspace-fluid staff-workspace-flex-column" data-testid="admin-settings-page" style={{ padding: '16px', background: '#f5f7fa', minHeight: '100%', width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', background: '#fff', padding: '12px 16px', borderRadius: '8px', border: '1px solid #f0f0f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          <span className="staff-eyebrow" style={{ fontSize: '14px', fontWeight: 600 }}>Cấu hình</span>
          ...
          items={[
            {
              ...
              children: (
                <div style={{ padding: '16px', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <Row gutter={[16, 16]}>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Hoạt động" value={summary.active} /></Card></Col>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Mặc định" value={summary.defaults} /></Card></Col>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Đã có giờ mở cửa" value={summary.withBusinessHours} /></Card></Col>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Có lịch đóng" value={summary.withClosures} /></Card></Col>
                  </Row>
                      ...
                      <div className="staff-admin-branch-list">
                        {branches.map((branch) => (
                          <button
                            ...
                            onClick={() => { setSelectedBranchId(branch.branch_id); setDetailDrawerOpen(true); }}
                          >
            ...
            },
            {
              ...
              children: (
                <div style={{ padding: '16px', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <Row gutter={[16, 16]}>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Bàn" value={tableSummary.total} /></Card></Col>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Có thể xếp khách" value={tableSummary.allocatable} /></Card></Col>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Đang liên kết live" value={tableSummary.operationallyLinked} /></Card></Col>
                    <Col xs={24} md={6}><Card className="staff-admin-summary-card"><Statistic title="Chi nhánh có bàn" value={tableSummary.branchScoped} /></Card></Col>
                  </Row>
                      ...
                      <div className="staff-admin-surface-list">
                        {tables.map((table) => (
                          <button
                            ...
                            onClick={() => { setSelectedTableId(table.table_id); setDetailDrawerOpen(true); }}
                          >
                            <div className="staff-admin-branch-row-main">
                      ...
                      <div className="staff-admin-surface-list">
                        {stations.map((station) => (
                          <button
                            ...
                            onClick={() => { setSelectedStationId(station.station_id); setDetailDrawerOpen(true); }}
                          >
                            <div className="staff-admin-branch-row-main">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {activeTab === 'kitchen' && (
            <Card className="staff-workspace-detail-card" title="Trạm bếp đang chọn">
              {!selectedStation ? (
                <EmptyBlock title="Chưa chọn trạm bếp" description="Chọn một trạm bếp để xem chi tiết." />
              ) : (
