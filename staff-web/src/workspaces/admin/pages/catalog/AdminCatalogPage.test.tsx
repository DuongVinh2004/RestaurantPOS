...
describe('AdminCatalogPage', () => {
  beforeEach(() => {
    ...
    apiMocks.listAdminMenuCategories.mockResolvedValue({
      data: [
        { category_id: 3, name: 'Breakfast', description: null, sort_order: 1, is_deleted: false },
      ],
    });
      ...
      data: [
        {
          ...
          category: { category_id: 3, name: 'Breakfast', description: null, sort_order: 1, is_deleted: false },
    ...
    apiMocks.importAdminMasterData.mockResolvedValue({
      data: {
        ...
        rows: [{ row_number: 1, status: 'valid', operation: 'create', errors: [], before: null, after: { name: 'Lunch' } }],
        commit: null,
      },
    });
  });
  ...
  it('renders catalog reads and loads selected item price rows', async () => {
    ...
    expect(await screen.findByText('Menu and pricing lane')).toBeInTheDocument();
    ...
    expect(await screen.findByText('45,000 VND')).toBeInTheDocument();
  });

  it('previews imports before commit can be sent with an idempotency key', async () => {
    renderPage();

    fireEvent.change(await screen.findByLabelText('Admin import JSON rows'), {
      target: { value: '[{"name":"Lunch"}]' },
    });
    ...
    expect(await screen.findByText(/Commit will send Idempotency-Key/)).toBeInTheDocument();
  });
});
