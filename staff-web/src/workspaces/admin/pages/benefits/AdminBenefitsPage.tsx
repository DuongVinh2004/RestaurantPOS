import { Button, Card, Col, Input, Row, Select, Space, Statistic, Switch, Typography, Drawer } from 'antd';
...
export function AdminBenefitsPage() {
  ...
  const [drawerMode, setDrawerMode] = useState<'voucher' | 'tier' | 'settings'>('voucher');
  ...
  return (
    <div className="staff-workspace-fluid staff-workspace-flex-column" data-testid="admin-benefits-page" style={{ padding: '16px', background: '#f5f7fa', minHeight: '100%', width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', background: '#fff', padding: '12px 16px', borderRadius: '8px', border: '1px solid #f0f0f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          <span className="staff-eyebrow" style={{ fontSize: '14px', fontWeight: 600 }}>Admin benefits</span>
          <StatusChip label={`${vouchers.length} Voucher`} tone="processing" />
          <StatusChip label={`${tiers.length} Hạng thân thiết`} tone="processing" />
          <StatusChip label={`${settings.length} Cấu hình`} tone="default" />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          <Button type="primary" onClick={() => { setDrawerMode('settings'); setDetailDrawerOpen(true); }}>Cấu hình & Export</Button>
        </div>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1, overflowY: 'auto' }}>
        <Row gutter={[16, 16]}>
          <Col xs={24} md={8}><Card className="staff-workspace-summary-card"><Statistic title="Voucher" value={vouchers.length} /></Card></Col>
          <Col xs={24} md={8}><Card className="staff-workspace-summary-card"><Statistic title="Hạng thân thiết" value={tiers.length} /></Card></Col>
          <Col xs={24} md={8}><Card className="staff-workspace-summary-card"><Statistic title="Cấu hình" value={settings.length} /></Card></Col>
        </Row>
        ...
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={12}>
            <Card
              ...
              extra={(
                <Button size="small" type="primary" onClick={() => { setSelectedVoucherId(null); setVoucherForm({ code: '', discountType: 'Fixed', discountValue: '', description: '', active: true }); setDrawerMode('voucher'); setDetailDrawerOpen(true); }}>
                  Tạo voucher
                </Button>
              )}
            >
          ...
          <Col xs={24} lg={12}>
            <Card
              title="Hạng khách thân thiết"
              className="staff-workspace-table-card"
              extra={(
                <Button size="small" type="primary" onClick={() => { setSelectedTierId(null); setTierForm({ code: '', name: '', minPoints: '', active: true }); setDrawerMode('tier'); setDetailDrawerOpen(true); }}>
                  Tạo hạng
                </Button>
              )}
            >
      ...
      <Drawer
        title={drawerMode === 'voucher' ? 'Voucher' : drawerMode === 'tier' ? 'Hạng khách thân thiết' : 'Cấu hình & Export'}
        placement="right"
        width={450}
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {drawerMode === 'voucher' && (
            <Card title={selectedVoucher ? "Cập nhật voucher" : "Tạo voucher mới"} className="staff-workspace-detail-card">
              <Space orientation="vertical" size={12} style={{ width: '100%' }}>
                <Input aria-label="Mã voucher" placeholder="Mã voucher" value={voucherForm.code} onChange={(event) => setVoucherForm((current) => ({ ...current, code: event.target.value }))} disabled={!!selectedVoucher} />
                <Select<DiscountType> aria-label="Loại giảm giá" value={voucherForm.discountType} options={discountTypeOptions} onChange={(value) => setVoucherForm((current) => ({ ...current, discountType: value }))} disabled={!!selectedVoucher} />
                ...
                <Space wrap>
                  {!selectedVoucher && <Button type="primary" onClick={() => createVoucherMutation.mutate()} loading={createVoucherMutation.isPending}>Tạo voucher</Button>}
                  {selectedVoucher && <Button type="primary" onClick={() => updateVoucherMutation.mutate()} loading={updateVoucherMutation.isPending}>Cập nhật voucher</Button>}
                </Space>
              </Space>
            </Card>
          )}
          ...
          {drawerMode === 'tier' && (
            <Card title={selectedTier ? "Cập nhật hạng" : "Tạo hạng mới"} className="staff-workspace-detail-card">
              <Space orientation="vertical" size={12} style={{ width: '100%' }}>
                <Input aria-label="Mã hạng" placeholder="Mã hạng" value={tierForm.code} onChange={(event) => setTierForm((current) => ({ ...current, code: event.target.value }))} disabled={!!selectedTier} />
                ...
                <Space wrap>
                  {!selectedTier && <Button type="primary" onClick={() => createTierMutation.mutate()} loading={createTierMutation.isPending}>Tạo hạng</Button>}
                  {selectedTier && <Button type="primary" onClick={() => updateTierMutation.mutate()} loading={updateTierMutation.isPending}>Cập nhật hạng</Button>}
                </Space>
              </Space>
            </Card>
          )}
          ...
          {drawerMode === 'settings' && (
            <>
              <Card title="Cấu hình ưu đãi" className="staff-workspace-detail-card">
                <Space orientation="vertical" size={12} style={{ width: '100%' }}>
                  <Select<BenefitSettingKey> aria-label="Khóa cấu hình ưu đãi" value={settingForm.key} options={benefitSettingOptions} onChange={(value) => setSettingForm((current) => ({ ...current, key: value }))} style={{ width: '100%' }} />
              ...
              <Card title="Export ưu đãi" className="staff-workspace-detail-card">
                <Space orientation="vertical" size={12} style={{ width: '100%' }}>
                  <Space wrap>
