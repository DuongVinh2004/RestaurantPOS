...
export function ReportingHubPage() {
  ...
  return (
    <div className="staff-workspace-fluid staff-workspace-flex-column" data-testid="reporting-hub-page" style={{ padding: '16px', background: '#f5f7fa', minHeight: '100%', width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', background: '#fff', padding: '12px 16px', borderRadius: '8px', border: '1px solid #f0f0f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
          <span className="staff-eyebrow" style={{ fontSize: '14px', fontWeight: 600 }}>Báo cáo Mộc Sen</span>
          ...
          <span style={{ fontSize: '12px', color: '#888' }}>{latestRefreshLabel}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          {can(session, 'settlement.manage') ? (
            <Button size="small" onClick={() => navigate(staffRoutePaths.ops.financeReview)}>Rà soát TC</Button>
          ) : null}
          {can(session, 'cashier.shift.manage') ? (
            <Button size="small" onClick={() => navigate(staffRoutePaths.ops.cashierShift)}>Ca thu ngân</Button>
          ) : null}
          {can(session, 'payment.refund') ? (
            <Button size="small" onClick={() => navigate(staffRoutePaths.ops.refunds)}>Hoàn tiền</Button>
          ) : null}
          {can(session, 'audit.view') ? (
            <Button size="small" onClick={() => navigate(staffRoutePaths.admin.auditTrail)}>Nhật ký</Button>
          ) : null}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
        {/* Filters */}
        <Card title="Bộ lọc" className="staff-workspace-filter-card" extra={filtersActive ? <Button size="small" onClick={resetFilters}>Đặt lại</Button> : null} style={{ flex: 1, minWidth: '300px' }}>
          <Row gutter={[12, 12]}>
            <Col xs={12} md={6}>
        ...
        <Card title="Tóm tắt" style={{ flex: 1, minWidth: '300px' }}>
          {activeTab === 'sales' ? (
            <Row gutter={[12, 12]}>
              <Col span={8}><Statistic title="Thực thu" value={salesSummary.netPaidAmount} formatter={(value) => formatMoney(Number(value ?? 0), filters.currency || 'VND')} /></Col>
              <Col span={8}><Statistic title="Tổng bill" value={salesSummary.grossBillAmount} formatter={(value) => formatMoney(Number(value ?? 0), filters.currency || 'VND')} /></Col>
              <Col span={8}><Statistic title="Hóa đơn" value={salesSummary.invoiceCount} /></Col>
            </Row>
          ) : activeTab === 'operations' ? (
            <Row gutter={[12, 12]}>
              <Col span={8}><Statistic title="Hoàn tất PV" value={operationsSummary.completedCount} /></Col>
              <Col span={8}><Statistic title="Khách chờ" value={operationsSummary.waitingSeatedCount} /></Col>
              <Col span={8}><Statistic title="Vòng quay TB" value={operationsSummary.avgTurnMinutes ?? 0} formatter={(value) => formatMinutes(Number(value ?? 0))} /></Col>
            </Row>
          ) : (
            <Row gutter={[12, 12]}>
              <Col span={8}><Statistic title="Nhập/xuất" value={inventorySummary.movementCount} /></Col>
              <Col span={8}><Statistic title="Biến động" value={inventorySummary.netQuantityDelta} formatter={(value) => formatQuantity(Number(value ?? 0))} /></Col>
              <Col span={8}><Statistic title="Hao hụt" value={inventorySummary.wastageQuantity} formatter={(value) => formatQuantity(Number(value ?? 0))} /></Col>
            </Row>
          )}
        </Card>
      </div>
          ...
          items={[
            {
              ...
              children: (
                <ReportingTabTable
                  ...
                  onSelectRow={(row) => { setSelectedRowKey(getRowKey('sales', row)); setDetailDrawerOpen(true); }}
                  columns={[
            ...
            },
            {
              ...
              children: (
                <ReportingTabTable
                  ...
                  onSelectRow={(row) => { setSelectedRowKey(getRowKey('operations', row)); setDetailDrawerOpen(true); }}
                  columns={[
                    { title: 'Ngày', render: (_, row) => row.business_date ?? 'Không có' },
                    { title: 'Chi nhánh', render: (_, row) => row.branch?.branch_code ?? `#${row.branch_id}` },
            ...
            },
            {
              ...
              children: (
                <ReportingTabTable
                  ...
                  onSelectRow={(row) => { setSelectedRowKey(getRowKey('inventory', row)); setDetailDrawerOpen(true); }}
                  columns={[
                    { title: 'Ngày', render: (_, row) => row.business_date ?? 'Không có' },
                    { title: 'Chi nhánh', render: (_, row) => row.branch?.branch_code ?? `#${row.branch_id}` },
      ...
      <Drawer
        title="Chi tiết ảnh chụp báo cáo"
        placement="right"
        width={450}
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <Card title="Tình trạng snapshot">
