...
describe('OrdersPage', () => {
  ...
  it('creates an order from the selected table reservation row_version', async () => {
    ...
    expect(apiMocks.listMenuItems).toHaveBeenCalledWith(
  ...
  it('uses canonical reservation lookup to load historical order candidates', async () => {
    ...
    await waitFor(() => expect(apiMocks.getOrderDetail).toHaveBeenCalledWith(9103));
  });
...
function arrangeOrderFixtures({
  ...
  apiMocks.listMenuItems.mockResolvedValue({
    data: [
      {
        item_id: 501,
        code: 'MI-501',
