...
import { formatDateTime, formatMoney, formatRelativeAge } from '../../core/utils/format';
import {
  cashierShiftTone,
  conversationTone,
  kitchenTone,
...
import { conversationSummaryStats, conversationTitle } from '../conversations/conversation-inbox';
import { financeFlagLabels, summarizeFinance } from '../finance/finance-review';
import {
...
export function buildDashboardFocus(session: StaffSession): DashboardFocusModel {
  ...
  if (roleName.includes('cashier') || (capabilities.has('cashier.shift.manage') && capabilities.has('settlement.manage') && !capabilities.has('kitchen.manage'))) {
    return {
      ...
      priorityPaths: ['/cashier-shift', '/finance-review', '/checkout', '/conversations'],
    };
  }
  ...
  if (roleName.includes('kitchen') || (capabilities.has('kitchen.manage') && !capabilities.has('settlement.manage'))) {
    return {
      ...
      priorityPaths: ['/kitchen', '/orders', '/tables', '/waiting-list'],
    };
  }
  ...
  if (roleName.includes('manager') || (capabilities.has('reporting.view') && capabilities.has('cashier.shift.manage') && capabilities.has('conversation.manage'))) {
    return {
      ...
      priorityPaths: ['/dashboard', '/finance-review', '/cashier-shift', '/reporting'],
    };
  }
  ...
  if (roleName.includes('supervisor') || (capabilities.has('conversation.manage') && capabilities.has('reservation.manage'))) {
    return {
      ...
      priorityPaths: ['/waiting-list', '/reservations', '/conversations', '/tables'],
    };
  }
  ...
  return {
    ...
    priorityPaths: ['/tables', '/reservations', '/waiting-list', '/orders'],
  };
}
...
export function buildShiftHealthModel(
  ...
  if (!actions.some((action) => action.path === '/access')) {
    actions.push({
  ...
  if (canManageStaffCashierShift(session) && !actions.some((action) => action.path === '/cashier-shift')) {
    actions.push({
...
export function buildUrgentAlerts(args: {
  ...
  if (financeSummary.outstandingCount > 0) {
    alerts.push({
      ...
      path: financeSummary.discrepancyCount > 0 ? '/finance-review?has_discrepancy=yes' : '/finance-review',
  ...
  if (conversationStats.unassigned > 0) {
    alerts.push({
      ...
      path: '/conversations?status=Open&assignment=unassigned',
  ...
  if (unassignedReservations > 0) {
    alerts.push({
      ...
      path: '/reservations?bucket=today',
  ...
  return {
    ...
    items: [
      ...reservationRows.slice(0, 2).map((row) => ({
        ...
        path: `/reservations?bucket=today&reservation=${row.reservation_id}`,
        actionLabel: 'Mở lượt',
      })),
...
export function buildCheckoutSnapshot(financeRows: Array<FinancialReconciliationRow>): DashboardSnapshotModel {
  ...
  return {
    ...
    items: financeRows.slice(0, 4).map((row) => ({
      ...
      path: `/finance-review?focus=${row.reservation.reservation_id}`,
      actionLabel: 'Mở bill',
    })),
...
export function buildConversationSnapshot(
  ...
  return {
    ...
    items: rows.slice(0, 4).map((row) => ({
      ...
      path: `/conversations?status=Open&conversation=${row.conversation_id}`,
      actionLabel: 'Mở hội thoại',
    })),
