import type { AuditTrailEntry, AuditTrailQuery } from '../../shared/api/staff-api';
...
export function auditLinkedEntityTarget(
  ...
  if (reservationId && permissions.canManageReservations) {
    return {
      kind: 'reservation',
      id: reservationId,
      path: `${staffRoutePaths.ops.reservations}?${buildJourneySearch({
        source: 'audit',
        reservationId,
      })}`,
    };
  ...
  if (orderId && permissions.canManageOrders) {
    return {
      kind: 'order',
      id: orderId,
      path: `${staffRoutePaths.ops.orders}?${buildJourneySearch({
        source: 'audit',
        orderId,
      })}`,
    };
