...
- Shared seams touched: SQL-first schema/dump/config/verifier/docs and `BuildsBookingScenario` were updated only for the B12 snapshot contract. Existing dirty B01-B07 money, provider, bootstrap and test-fixture edits remain outside the B12 patch. A frozen-manifest refresh was attempted from the isolated committed B12 tree and correctly failed closed because the base branch's generated TypeScript SDK is already stale relative to its OpenAPI artifact; B12 does not absorb that unrelated generated-artifact delta.
- Remaining risks: pre-existing legacy order items can only be backfilled with the recipe visible at deployment because no historical version exists; the patch documents this approximation and freezes it before new code deploys. The frozen release manifest remains stale until the API-contract artifact owner regenerates the pre-existing SDK/OpenAPI set, after which patch 71 hashes must be frozen. Clean-tree `test:orders` and `test:release-contract` also expose existing B02/B05 baseline expectation drift, while the integrated worktree gates are green; B12-specific, inventory and kitchen clean-tree lanes pass. Runtime proof covered MySQL and the touched inventory/order/KDS slice, not Redis or scheduler because B12 adds no Redis/scheduled behavior. B07 remains externally blocked, B08 remains prohibited, B13 remains dependency-blocked by B09, and production remains `NO-GO`.
