...
class CreateReservationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'guest_name' => [
                $isStaff || $actor->isCustomerSession() ? 'nullable' : 'prohibited',
                'string',
                'max:200',
                Rule::requiredIf(fn () => ($isStaff || $actor->isCustomerSession()) && ! $this->filled('user_id')),
                Rule::prohibitedIf(fn () => ($isStaff || $actor->isCustomerOwner()) && $this->filled('user_id')),
            ],
            'guest_phone' => [
                $isStaff || $actor->isCustomerSession() ? 'nullable' : 'prohibited',
                'string',
