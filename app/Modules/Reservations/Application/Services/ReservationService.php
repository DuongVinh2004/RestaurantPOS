        ...
        $runner = function () use ($payload, $actorUserId, $startUtc, $endUtc, $tableIds, $holdId, $sessionId, $trustedHoldIds, $policyNowUtc, $policyUseCase) {
            return DB::transaction(function () use ($payload, $actorUserId, $startUtc, $endUtc, $tableIds, $holdId, $sessionId, $trustedHoldIds, $policyNowUtc, $policyUseCase) {
                ...
                if ($preparedPreorder !== null) {
                    ...
                    // Đồng bộ sang bảng preorders cho luồng Customer self-service
                    ...
                    foreach ($normalizedPreOrderItems as $row) {
                        ...
                        // Đồng bộ sang bảng preorder_items
                        DB::table('preorder_items')->insert([
                            'preorder_id' => $preorderId,
                            'menu_item_id' => (int) $row['item_id'],
                            'quantity' => $quantity,
                            'unit_price_snapshot' => Money::formatMinor($unitPriceMinor),
                            'line_total_snapshot' => Money::formatMinor($unitPriceMinor * $quantity),
                            'currency' => $currency,
