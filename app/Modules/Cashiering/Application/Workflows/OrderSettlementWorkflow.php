...
class OrderSettlementWorkflow
{
        ...
        $result = $this->locks->withLockKeys($context['lock_keys'], function () use (
            ...
            return DB::transaction(function () use (
                ...
                $order = $this->paymentCaptureService->executeLocked(
                    ...
                    findExistingOrderPaymentReplay: fn (int $replayOrderId, string $replayKey): ?ReservationOrder => $this->findExistingOrderPaymentReplay($replayOrderId, $replayKey, $staffUserId),
    ...
    public function payOrder(
        ...
        if ($idempotencyKey !== '') {
            ...
            if (is_numeric($hit)) {
                ...
                if ($existing) {
                    $this->assertSettlementOrderBranchAccessible($existing, $staffUserId);
    ...
    private function payOrderLocked(
        ...
        string $cashieringReplayScope = CashieringReplayRecorder::SCOPE_STAFF_PAY_ORDER,
    ...
    private function executeRefundFlow(
        ...
        if ($idempotencyKey !== '') {
            ...
            $replayed = $this->findExistingRefundReplay($reservationId, $idempotencyKey, $staffUserId);
            if ($replayed !== null) {
                return $replayed;
            }
        }
        ...
        try {
            $result = DB::transaction(function () use (
                ...
                $result = $this->refundExecutionService->executeLocked(
                    ...
                    releaseAppliedVoucherLocked: fn (Reservation $lockedReservation, Collection $lockedOrders, ?int $actorUserId, bool $detachReservation = true): float => $this->releaseAppliedVoucherLocked($lockedReservation, $lockedOrders, $actorUserId, $detachReservation),
    ...
    private function assertReservationBranchAccessible(Reservation $reservation, ?int $staffUserId): int
    {
        $branchId = $reservation->branch_id !== null && $reservation->branch_id !== ''
            ? $this->branchContextService->resolveBranchId($reservation->branch_id, false)
            : $this->branchContextService->resolveBranchId(null, false);
    ...
    private function assertSettlementOrderBranchAccessible(ReservationOrder $order, ?int $staffUserId): void
    {
        ...
        if (! $reservation instanceof Reservation && (int) ($order->reservation_id ?? 0) > 0) {
            $reservation = Reservation::query()->find((int) $order->reservation_id);
        }

        if ($reservation instanceof Reservation) {
            $this->assertReservationBranchAccessible($reservation, $staffUserId);
    ...
    private function findExistingCheckoutReplay(
        ...
        $order = $this->findExistingOrderPaymentReplay($orderId, $idempotencyKey, $staffUserId)
            ?? $this->findExistingOrderCashieringReplay($orderId, $idempotencyKey, $cashieringReplayScope, $staffUserId);
        if (! $order) {
            return null;
        }

        $this->assertSettlementOrderBranchAccessible($order, $staffUserId);

        return $this->buildCheckoutResponse($order, $fallbackCurrency);
    }

    private function findExistingOrderPaymentReplay(int $orderId, string $idempotencyKey, ?int $staffUserId = null): ?ReservationOrder
    {
        $idempotencyKey = trim($idempotencyKey);
        if ($idempotencyKey === '') {
            return null;
        ...
        if (! $hydrated instanceof ReservationOrder) {
            return null;
        }

        $this->assertSettlementOrderBranchAccessible($hydrated, $staffUserId);

        return $this->attachTotals($hydrated);
    }

    private function findExistingOrderCashieringReplay(int $orderId, string $idempotencyKey, string $cashieringReplayScope, ?int $staffUserId = null): ?ReservationOrder
    {
        $record = $this->cashieringReplayRecorder->find(
            scope: $cashieringReplayScope,
    ...
    private function recordCheckoutCashieringReplay(string $scope, int $orderId, string $idempotencyKey, ?string $requestFingerprint): void
    {
        $this->cashieringReplayRecorder->recordSuccess(
            scope: $scope,
            aggregateType: 'reservation_order',
            aggregateId: $orderId,
            idempotencyKey: $idempotencyKey,
    ...
    private function assertCheckoutReplayMatchesRequest(
        ...
        ?string $cashieringReplayScope = null
    ): void {
        $idempotencyKey = trim($idempotencyKey);
        if ($idempotencyKey === '') {
            return;
        ...
        if ($cashieringReplayScope !== null) {
            $this->cashieringReplayRecorder->assertReplayMatches(
                scope: $cashieringReplayScope,
                aggregateType: 'reservation_order',
                aggregateId: $orderId,
                idempotencyKey: $idempotencyKey,
     ...
     * @return array<string,mixed>|null
     */
    private function findExistingRefundReplay(int $reservationId, string $idempotencyKey, ?int $staffUserId = null): ?array
    {
        $idempotencyKey = trim($idempotencyKey);
        if ($idempotencyKey === '') {
            return null;
