...
class CustomerWaitingListService
{
    ...
    public function createEntry(int $ownerUserId, array $payload): WaitlistEntry
    {
        return $this->withOwnerActiveEntryLock($ownerUserId, function () use ($ownerUserId, $payload) {
            return DB::transaction(function () use ($ownerUserId, $payload) {
                $hasActiveEntry = WaitlistEntry::query()
                    ->where('user_id', $ownerUserId)
    ...
    public function getOwnerEntryOrFail(int $waitingId, int $ownerUserId): WaitlistEntry
    {
    ...
    public function acceptEntry(int $waitingId, int $ownerUserId, ?int $expectedRowVersion = null): WaitlistEntry
    {
        return $this->withWaitingEntryLock($waitingId, function () use ($waitingId, $ownerUserId, $expectedRowVersion) {
            return DB::transaction(function () use ($waitingId, $ownerUserId, $expectedRowVersion) {
                ...
                app(OperationalRealtimeService::class)->publishWaitingListEvent('waiting_list.customer_accepted', [
                    'waiting_id' => (int) $entry->waiting_id,
                    'notify_expires_at' => $entry->notify_expires_at?->copy()?->utc()?->toIso8601String(),
                ]);
                app(OperationalRealtimeService::class)->publishBoardEvent('waiting_list.customer_accepted', [
                    'waiting_id' => (int) $entry->waiting_id,
                ], ['board']);
    ...
    public function confirmArrivalEntry(int $waitingId, int $ownerUserId, ?int $expectedRowVersion = null): WaitlistEntry
    {
        return $this->withWaitingEntryLock($waitingId, function () use ($waitingId, $ownerUserId, $expectedRowVersion) {
            return DB::transaction(function () use ($waitingId, $ownerUserId, $expectedRowVersion) {
                $entry = $this->lockOwnerEntryForUpdate($waitingId, $ownerUserId);
                ...
                app(OperationalRealtimeService::class)->publishWaitingListEvent('waiting_list.customer_arrival_confirmed', [
                    'waiting_id' => (int) $entry->waiting_id,
                ]);
                app(OperationalRealtimeService::class)->publishBoardEvent('waiting_list.customer_arrival_confirmed', [
                    'waiting_id' => (int) $entry->waiting_id,
                ], ['board']);
    ...
    public function declineEntry(int $waitingId, int $ownerUserId, ?int $expectedRowVersion = null): WaitlistEntry
    {
        return $this->withWaitingEntryLock($waitingId, function () use ($waitingId, $ownerUserId, $expectedRowVersion) {
            return DB::transaction(function () use ($waitingId, $ownerUserId, $expectedRowVersion) {
                $entry = $this->lockOwnerEntryForUpdate($waitingId, $ownerUserId);
                ...
                WaitlistInvitationStateMachine::applyCustomerDeclined($entry, $now, $ownerUserId);
                $entry->save();
                ...
                app(OperationalRealtimeService::class)->publishWaitingListEvent('waiting_list.customer_declined', [
                    'waiting_id' => (int) $entry->waiting_id,
                ]);
                app(OperationalRealtimeService::class)->publishBoardEvent('waiting_list.customer_declined', [
                    'waiting_id' => (int) $entry->waiting_id,
                ], ['board']);
    ...
    public function cancelEntry(int $waitingId, int $ownerUserId, ?int $expectedRowVersion = null, ?string $cancelReason = null): WaitlistEntry
    {
        return $this->withWaitingEntryLock($waitingId, function () use ($waitingId, $ownerUserId, $expectedRowVersion, $cancelReason) {
            return DB::transaction(function () use ($waitingId, $ownerUserId, $expectedRowVersion, $cancelReason) {
                ...
                WaitlistInvitationStateMachine::applyCustomerCancelled(
                    $entry,
                    Carbon::now('UTC'),
                ...
                app(OperationalRealtimeService::class)->publishWaitingListEvent('waiting_list.customer_cancelled', [
                    'waiting_id' => (int) $entry->waiting_id,
                ]);
                app(OperationalRealtimeService::class)->publishBoardEvent('waiting_list.customer_cancelled', [
                    'waiting_id' => (int) $entry->waiting_id,
                ], ['board']);
    ...
    private function lockOwnerEntryForUpdate(int $waitingId, int $ownerUserId): WaitlistEntry
    {
        $entry = WaitlistEntry::query()
            ->whereKey($waitingId)
            ->where('user_id', $ownerUserId)
    ...
    private function assertOpenNotifiedWindow(WaitlistEntry $entry, ?Carbon $now = null): void
    {
        $now ??= Carbon::now('UTC');
    ...
    private function cancelExistingNotifyHold(WaitlistEntry $entry, ?int $ownerUserId = null): void
    {
        $now = Carbon::now('UTC');
    ...
    private function throwOwnerMutationEntryNotFound(): never
    {
        ...
        $exception->setModel(WaitlistEntry::class);
