...
use App\Services\PaymentIntegration\PaymentProviderSessionScopeGuard;
use App\Services\PaymentIntegration\ReservationDepositPaymentSessionLifecycleService;
use App\Services\Reservation\ReservationDepositReadService;
use App\Services\Reservation\ReservationDepositRealtimePublisher;
use App\Services\ReservationDepositPaymentService;
...
class CustomerReservationDepositPaymentService
{
    public function __construct(
        ...
        private readonly ReservationDepositPaymentSessionLifecycleService $sessionLifecycle,
        ...
        return $this->locks->withReservationLock($reservationId, function () use ($reservationId, $payload, $customerUserId, $sessionId, $idempotencyKey) {
            return DB::transaction(function () use ($reservationId, $payload, $customerUserId, $sessionId, $idempotencyKey) {
                ...
                $this->sessionScopeGuard->assertProviderSessionCodeIsAvailable(
                    $providerCode,
                    $providerSessionCode,
                    \App\Enums\PaymentSessionScope::Deposit,
                );
    ...
    private function assertReservationSupportsCustomerDepositPayment(Reservation $reservation, array $paymentSummary): void
    {
        ...
        if ((float) ($paymentSummary['final_captured_amount'] ?? 0.0) > 0.0001) {
            throw ValidationExceptionFactory::make([
                'deposit' => ['Reservation already has final settlement captured; deposit payment is not allowed.'],
            ]);
        }

        if ($this->calculateOutstandingDeposit($reservation, $paymentSummary) <= 0.0001) {
            throw ValidationExceptionFactory::make([
