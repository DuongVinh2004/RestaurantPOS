...
class CoreOpsGateService
{
    ...
    {
        $relativeDefinitionPath = trim((string) config('booking_release.core_ops_gate.definition_path', 'tests/fixtures/core_ops_gate_suite.json'));
        $definitionAbsolutePath = base_path($relativeDefinitionPath);

        if (! File::exists($definitionAbsolutePath)) {
            throw new \RuntimeException(sprintf('Core ops gate definition [%s] is missing.', $relativeDefinitionPath));
        }

        $raw = File::get($definitionAbsolutePath);
        $decoded = json_decode($raw, true);
        if (! is_array($decoded)) {
            throw new \RuntimeException(sprintf('Core ops gate definition [%s] is not valid JSON.', $relativeDefinitionPath));
        }
        ...
        if ($suite === '' || $tests === []) {
            throw new \RuntimeException(sprintf('Core ops gate definition [%s] is missing suite metadata or test entries.', $relativeDefinitionPath));
        }

        return [
            'suite' => $suite,
            'description' => $description !== '' ? $description : 'Canonical core ops booking gate.',
            'definition_path' => $relativeDefinitionPath,
            'snapshot_path' => trim((string) config('booking_release.core_ops_gate.snapshot_path', 'storage/app/booking_release/core_ops_gate_snapshot.json')),
    ...
    protected function runSingleTest(array $test): array
    {
        ...
        try {
            ...
            if (! File::exists($artisanPath)) {
                throw new \RuntimeException('Missing artisan bootstrap; cannot execute core ops gate tests.');
            }
