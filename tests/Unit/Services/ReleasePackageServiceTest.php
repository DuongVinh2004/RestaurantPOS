...
class ReleasePackageServiceTest extends TestCase
{
    ...
    public function test_package_streams_large_payloads_into_archive_and_inventory(): void
    {
        $schemaPath = base_path($this->root.'/schema.sql');
        $packageFile = base_path($this->root.'/src/large-release.bin');
        File::ensureDirectoryExists(dirname($schemaPath));
        File::ensureDirectoryExists(dirname($packageFile));
        File::put($schemaPath, "alpha\nbeta\n");
        File::put($packageFile, str_repeat('0123456789abcdef', 196608));
        ...
        config()->set('booking_release.packaging.package_prefix', 'booking-test-release');
        config()->set('booking_release.packaging.include_paths', [
            ['path' => $this->root.'/src/large-release.bin', 'required' => true],
        ]);
        config()->set('booking_release.packaging.sidecars.latest_pointer_path', $this->root.'/build/latest-package.json');
        ...
        $report = app(ReleasePackageService::class)->package('streaming-payload', verifyFrozen: true, overwrite: true);
        ...
        $entry = collect((array) ($inventory['entries'] ?? []))
            ->firstWhere('source_path', $this->root.'/src/large-release.bin');

        $this->assertIsArray($entry);
        $this->assertSame(3145728, $entry['bytes'] ?? null);
        $this->assertSame(hash_file('sha256', $packageFile), $entry['sha256'] ?? null);
        $this->assertSame(1, $entry['line_count'] ?? null);
    }
