...
class StaffTableBoardFeatureTest extends TestCase
{
    ...
    public function test_staff_table_board_keeps_confirmed_hold_visible_even_after_expire_at_has_elapsed(): void
    {
        $staffId = $this->createUser(['role_name' => 'Staff']);
        $headers = $this->staffAuthHeaders($staffId);
        ...
        $table = collect($response->json('data'))->firstWhere('table_id', $heldTableId);

        self::assertNotNull($table);
        self::assertSame('held_in_range', data_get($table, 'board_state'));
        self::assertSame('Confirmed', data_get($table, 'hold.hold_status'));
    }
