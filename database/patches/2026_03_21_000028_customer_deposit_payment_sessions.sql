...
CREATE TABLE IF NOT EXISTS `reservation_deposit_payment_sessions` (
  ...
  CONSTRAINT `fk_reservation_deposit_payment_sessions__reservation_id_5da21e5a` FOREIGN KEY (`reservation_id`) REFERENCES `reservations` (`reservation_id`) ON DELETE CASCADE ON UPDATE RESTRICT,
  CONSTRAINT `fk_reservation_deposit_payment_sessions__customer_user_id__users` FOREIGN KEY (`customer_user_id`) REFERENCES `users` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT,
  CONSTRAINT `fk_reservation_deposit_payment_sessions__linked_payment_140895f5` FOREIGN KEY (`linked_payment_id`) REFERENCES `payments` (`payment_id`) ON DELETE SET NULL ON UPDATE RESTRICT,
  CONSTRAINT `fk_reservation_deposit_payment_sessions__created_by__users` FOREIGN KEY (`created_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL ON UPDATE RESTRICT,
  CONSTRAINT `fk_reservation_deposit_payment_sessions__updated_by__users` FOREIGN KEY (`updated_by`) REFERENCES `users` (`user_id`) ON DELETE SET NULL ON UPDATE RESTRICT,
  CONSTRAINT `chk_reservation_deposit_payment_sessions__money_nonneg` CHECK (`amount` >= 0),
  CONSTRAINT `chk_reservation_deposit_payment_sessions__session_status` CHECK (`session_status` in ('Created','Pending','Succeeded','Failed','Cancelled','Expired')),
  CONSTRAINT `chk_reservation_deposit_payment_sessions__settlement_status` CHECK (`settlement_status` in ('NotApplied','Applied','Skipped'))
